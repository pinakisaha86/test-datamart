    ...
    for tgt in tgt_list:
        ...
        if tgt == 'REGIS_DIM':
           ...
           # result.repartition(1).write.option("header", "true").mode("overwrite").parquet("")
            ...
            child_dim_df = spark.read.parquet('s3a://' + s3_bucket + '/' + staging_loc + '/' + tgt_conf['source_data'])

            child_dim_df.createOrReplaceTempView("CP")
            result1 = spark.sql(tgt_conf['loadingQuery'])

           # result.repartition(1).write.option("header", "true").mode("overwrite").parquet("")
            jdbc_url = get_redshift_jdbc_url(app_secret)
            print(jdbc_url)

            result1.coalesce(1).write \
                .format("io.github.spark_redshift_community.spark.redshift") \
                .option("url", jdbc_url) \
                .option("tempdir", "s3a://" + s3_bucket + "/temp") \
                .option("forward_spark_s3_credentials", "true") \
                .option("dbtable", "public.CHILD_DIM") \
                .mode("overwrite") \
                .save()
